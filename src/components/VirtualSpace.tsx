...
import { scenes, type SceneId, type ScenePortal } from "@/lib/scenes";
import dentistsPreview from "../../assets/scenes/dentists/dentists-scene-v1-preview.png";
import guidancePreview from "../../assets/scenes/guidance/guidance-portal-v4-preview.png";
import technologyPreview from "../../assets/scenes/technology/technology-scene-v1-preview.png";

type TransitionPhase = "idle" | "closing" | "waiting" | "opening";

type SpaceStyle = CSSProperties & {
  "--scene-accent": string;
  "--scene-accent-soft": string;
  "--portal-origin-x"?: string;
  "--portal-origin-y"?: string;
  "--portal-origin-w"?: string;
  "--portal-origin-h"?: string;
};
...
function getHeroPortalPreview(targetId: SceneId): StaticImageData | null {
  switch (targetId) {
    case "dentists":
      return dentistsPreview;
    case "guidance":
...
export function VirtualSpace({ initialSceneId }: { initialSceneId: SceneId }) {
  const router = useRouter();
  const reducedMotion = useReducedMotion();
  const [currentSceneId, setCurrentSceneId] = useState(initialSceneId);
  const [targetSceneId, setTargetSceneId] = useState<SceneId | null>(null);
  const [phase, setPhase] = useState<TransitionPhase>("idle");
  const [transitionOrigin, setTransitionOrigin] = useState<TransitionOrigin | null>(null);
  const transitionLock = useRef(false);
  const pendingTarget = useRef<SceneId | null>(null);
  const pointerFrame = useRef<number | null>(null);

  useEffect(() => {
    if (phase === "waiting" && pendingTarget.current === initialSceneId) {
    ...
    if (initialSceneId !== currentSceneId) {
      ...
      setCurrentSceneId(initialSceneId);
      window.scrollTo({ top: 0, behavior: "auto" });
      window.requestAnimationFrame(focusSceneContent);
    }
  }, [currentSceneId, initialSceneId, phase]);
  ...
  const targetScene = targetSceneId ? scenes[targetSceneId] : null;

  function navigateTo(targetId: SceneId, event: MouseEvent<HTMLAnchorElement>) {
    if (
    ...
    if (transitionLock.current || phase !== "idle") {
  ...
  function handleTransitionEnd(event: AnimationEvent<HTMLDivElement>) {
    ...
    if (phase === "closing" && pendingTarget.current) {
      const target = scenes[pendingTarget.current];
      setPhase("waiting");
  ...
  const style: SpaceStyle = {
    ...
    "--scene-accent-soft": scene.accentSoft,
    ...(transitionOrigin
      ? {
          "--portal-origin-x": `${transitionOrigin.x}px`,
          "--portal-origin-y": `${transitionOrigin.y}px`,
          "--portal-origin-w": `${transitionOrigin.width}px`,
          "--portal-origin-h": `${transitionOrigin.height}px`,
        }
      : {}),
  };
  ...
  return (
    <>
      <a className="skip-link" href="#scene-content">
        Ugrás a tér tartalmához
      </a>
      <main
        className="virtual-space"
        data-phase={phase}
        data-scene={scene.id}
        data-transition-target={targetSceneId ?? undefined}
        onPointerMove={handlePointerMove}
        style={style}
      >
        <div aria-hidden="true" className="space-atmosphere">
          <div className="space-grid" />
          <div className="space-horizon" />
          <div className="space-vignette" />
        </div>

        <div aria-busy={phase !== "idle"} className="scene-shell" inert={phase !== "idle"}>
          {scene.id === "hero" ? (
            <HeroScene onNavigate={navigateTo} />
          ) : (
            <ContentScene sceneId={scene.id} onNavigate={navigateTo} />
          )}
        </div>
        ...
        <div
          ...
          style={
            targetScene
              ? ({
                  "--target-accent": targetScene.accent,
                  "--target-soft": targetScene.accentSoft,
                } as CSSProperties)
              : undefined
          }
        >
          <div className="transition-portal__core" onAnimationEnd={handleTransitionEnd}>
            <span className="transition-portal__motif transition-portal__motif--dentists">
              <span className="transition-portal__mark">DTA</span>
              <span className="transition-portal__tooth" />
              <span className="transition-portal__flow" />
              <span className="transition-portal__node transition-portal__node--one" />
              <span className="transition-portal__node transition-portal__node--two" />
              <span className="transition-portal__node transition-portal__node--three" />
            </span>
            <span className="transition-portal__motif transition-portal__motif--guidance">
              <span className="transition-portal__reception" />
              <span className="transition-portal__search" />
              <span className="transition-portal__book transition-portal__book--left" />
              <span className="transition-portal__book transition-portal__book--right" />
              <span className="transition-portal__route" />
              <span className="transition-portal__node transition-portal__node--one" />
              <span className="transition-portal__node transition-portal__node--two" />
              <span className="transition-portal__node transition-portal__node--three" />
            </span>
            <span className="transition-portal__motif transition-portal__motif--technology">
              <span className="transition-portal__screen" />
              <span className="transition-portal__tech" />
              <span className="transition-portal__articulator" />
              <span className="transition-portal__printer" />
              <span className="transition-portal__rail" />
              <span className="transition-portal__node transition-portal__node--one" />
              <span className="transition-portal__node transition-portal__node--two" />
              <span className="transition-portal__node transition-portal__node--three" />
            </span>
        ...
        <p aria-live="polite" className="visually-hidden">
          {phase === "closing" && targetScene ? `Átlépés ide: ${targetScene.title}` : ""}
          {phase === "opening" ? `${scene.title} tér betöltve.` : ""}
        </p>
      </main>
    </>
  );
...
function focusSceneContent() {
  document.querySelector<HTMLElement>("#scene-content")?.focus({ preventScroll: true });
}
...
type GuidanceKnowledgeDraft = {
  ...
  relatedArticles?: readonly KnowledgeArticleDraft[];
  status: string;
  title: string;
};
...
} as const satisfies Record<(typeof guidanceTopicFamilies)[number]["tag"], GuidanceKnowledgeDraft>;

function HeroScene({
  onNavigate,
}: {
  onNavigate: (target: SceneId, event: MouseEvent<HTMLAnchorElement>) => void;
}) {
  return (
    <section className="hero-scene hero-scene--keyvisual" id="scene-content" aria-labelledby="hero-title" tabIndex={-1}>
      <h1 className="visually-hidden" id="hero-title">
        DTA Virtual Space – válasszon egy teret
      </h1>
      <div aria-hidden="true" className="hero-keyvisual">
        <div className="hero-keyvisual__image" />
        <div className="hero-keyvisual__shade hero-keyvisual__shade--top" />
        <div className="hero-keyvisual__shade hero-keyvisual__shade--bottom" />
        <div className="hero-keyvisual__shade hero-keyvisual__shade--right" />
      </div>
      <div className="hero-copy">
        <p className="hero-copy__claim">Mi állunk a mosolyok mögött.</p>
        <p className="hero-copy__guidance">Segítünk eligazodni a lehetőségek között.</p>
      </div>
      <div className="hero-hotspots" aria-label="Belépési portálok">
        {scenes.hero.portals.map((portal, index) => (
          <HeroPortalHotspot key={portal.target} index={index} portal={portal} onNavigate={onNavigate} />
        ))}
      </div>
    </section>
  );
...
function HeroPortalHotspot({
  ...
  const targetId = portal.target;
  if (!targetId) return null;
  const target = scenes[targetId];

  return (
    <Link
      aria-label={portal.accessibleDescription}
      className={`hero-hotspot hero-hotspot--${targetId}`}
      data-portal-index={index}
      href={target.path}
      onClick={(event) => onNavigate(targetId, event)}
    >
      <span aria-hidden="true" className="hero-hotspot__frame" />
      <span className="visually-hidden">{portal.accessibleDescription}</span>
    </Link>
  );
}
...
function Portal({
  ...
  if (!targetId) return null;
  const target = scenes[targetId];
  const previewImage = getHeroPortalPreview(targetId);
  ...
  return (
    <Link
      ...
      style={
        {
          "--portal-accent": target.accent,
          "--portal-soft": target.accentSoft,
        } as CSSProperties
      }
    >
      <span aria-hidden="true" className="portal__frame">
        <span className="portal__preview">
          {previewImage ? (
            <Image
              alt=""
              aria-hidden="true"
              className={`portal__preview-image portal__preview-image--${targetId}`}
              fill
              sizes="(max-width: 560px) 92vw, 16rem"
              src={previewImage}
            />
          ) : null}
          <span className="portal__preview-shade" />
        </span>
      </span>
      <span className="visually-hidden" id={`portal-description-${targetId}`}>
        {portal.accessibleDescription}
      </span>
    </Link>
  );
}
...
function ContentScene({
  ...
  sceneId: Exclude<SceneId, "hero">;
  onNavigate: (target: SceneId, event: MouseEvent<HTMLAnchorElement>) => void;
}) {
  const scene = scenes[sceneId];
  const plannedPortal = scene.portals.find((portal) => portal.status === "planned");
  const nextPortal = scene.portals.find(
    (portal) => portal.status === "active" && portal.target && portal.target !== "hero",
  );
  const externalPortal = scene.portals.find(
    (portal) => portal.status === "active" && portal.externalUrl,
  );
  const returnPortal = scene.portals.find(
    (portal) => portal.status === "active" && portal.target === "hero",
  );

  return (
    <article className={`content-scene content-scene--${sceneId}`} id="scene-content" tabIndex={-1}>
      <header className={`scene-intro scene-intro--${sceneId}`}>
        <div className="scene-intro__copy">
          <p className="scene-kicker">{scene.code}</p>
          <h1>{scene.title}</h1>
          <p>{scene.intro}</p>
        </div>
        <SceneVisual sceneId={sceneId} />
        <span aria-hidden="true" className="scroll-cue">
          Görgetés
        </span>
      </header>

      {sceneId === "guidance" ? <HostConsole onNavigate={onNavigate} /> : null}
      {sceneId === "guidance" ? <KnowledgePreview onNavigate={onNavigate} /> : null}
      {sceneId === "clinics" ? (
        <div aria-label="Saját rendelők rendelői útvonala" className="clinic-journey">
          <ClinicConsole />
          <ClinicOrientation />
          <ClinicPathway />
        </div>
      ) : null}
      {sceneId === "technology" ? <TechnologyConsole /> : null}
      {sceneId === "technology" ? <TechnologyPathway /> : null}

      <div className="story-track">
        {scene.stops.map((stop) => (
          <section
            className={`story-stop story-stop--${sceneId}`}
            data-stop-index={stop.index}
            id={`${sceneId}-stop-${stop.index}`}
            key={stop.index}
          >
            <div aria-hidden="true" className="story-stop__marker">
              {stop.index}
            </div>
            <div className="story-stop__copy">
              <p className="scene-kicker">Állomás {stop.index}</p>
              <h2>{stop.title}</h2>
              <p>{stop.body}</p>
            </div>
            <div aria-hidden="true" className="story-stop__volume">
              <span />
              <span />
              <span />
            </div>
      ...
      <footer className="scene-exit">
        {nextPortal?.target ? (
          <Link
            aria-label={nextPortal.accessibleDescription}
            className="future-portal future-portal--active"
            href={scenes[nextPortal.target].path}
            onClick={(event) => onNavigate(nextPortal.target!, event)}
          >
            <span className="scene-kicker">Következő tér</span>
            <strong>{nextPortal.label}</strong>
            <small>Belépés a portálon át</small>
          </Link>
        ) : externalPortal?.externalUrl ? (
          <a
            aria-label={externalPortal.accessibleDescription}
            className="future-portal future-portal--active future-portal--external"
            href={externalPortal.externalUrl}
            rel="noreferrer"
            target="_blank"
          >
            <span className="scene-kicker">Külön szakmai oldal</span>
            <strong>{externalPortal.label}</strong>
            <small>Szakmai fórum · bérmunka · együttműködés · szakmai támogatás</small>
          </a>
        ) : plannedPortal ? (
          <aside
            aria-label={plannedPortal.accessibleDescription}
            className={`future-portal future-portal--planned${
              plannedPortal.id === "clinics-to-booking" ? " future-portal--booking" : ""
            }${
              plannedPortal.id === "technology-to-flexi" ? " future-portal--external" : ""
            }`}
          >
            <span className="scene-kicker">
              {plannedPortal.id === "clinics-to-booking"
                ? "Zárt adatkapu"
                : plannedPortal.id === "technology-to-flexi"
                  ? "Külön szakmai oldal"
                  : "Következő tér"}
            </span>
            <strong>{plannedPortal.label}</strong>
            <small>
              {plannedPortal.id === "clinics-to-booking"
                ? "Későbbi funkció · jelenleg nincs adatbekérés"
                : plannedPortal.id === "technology-to-flexi"
                  ? "Működő honlap · anyagrendelés és kurzusjelentkezés · korszerűsítés későbbi menetben"
                : "Tervezett portál"}
            </small>
            {plannedPortal.id === "clinics-to-booking" ? <BookingGatePreview /> : null}
          </aside>
        ) : (
          <p className="scene-exit__note">A partneri kapcsolódás helye a következő tartalmi körben készül.</p>
        )}

        {returnPortal?.target ? (
          <Link
            className="return-link"
            href={scenes[returnPortal.target].path}
            onClick={(event) => onNavigate(returnPortal.target!, event)}
          >
            <span aria-hidden="true">←</span> {returnPortal.label}
          </Link>
        ) : null}
      </footer>
    </article>
...
function BookingGatePreview() {
  const clinicRoutes = [
    {
      id: "private-ozd",
      badge: "Kiemelt · Magánrendelő",
      title: "Magánrendelő Ózd · 2 szék",
      note:
        "A legkiemeltebb, prémium rendelői irány. Nem külön térbe visz, hanem a közös előszobán belül kap hangsúlyosabb továbbvezetést.",
      tone: "Prémium, nyugodt, célzott",
      focus: "Gyorsabb és személyesebb továbbvezetés",
      nextStep: "A közös előszobából kiemelt konzultációs ritmus felé billenhet tovább.",
      consult:
        "A konzultációs előszoba a kiemelt magánrendelő irányához igazodik, nyugodt és célzott továbbvezetéssel.",
      booking:
        "Az időpontkérés később ehhez a kiemelt ózdi magánrendelői ritmushoz illeszkedhet.",
      featured: true,
    },
    {
      id: "neak-kazincbarcika",
      badge: "NEAK · Kazincbarcika",
      title: "NEAK rendelő Kazincbarcika",
      note:
        "Külön városban működő NEAK irány. A helyszínválasztást előbb tisztázni kell, hogy a látogató ne rossz város felé haladjon tovább.",
      tone: "Tisztázó, helyszíntudatos",
      focus: "Kazincbarcika mint külön városi döntési pont",
      nextStep: "A következő lépés előtt világossá kell váljon, hogy nem az ózdi NEAK út felé tart.",
      consult:
        "A konzultációs előszoba a kazincbarcikai NEAK útvonalhoz igazodó, helyszíntudatos eligazítást adhat.",
      booking:
        "Az időpontkérés később a kazincbarcikai NEAK rendelő saját működési logikájához igazodhat.",
      featured: false,
    },
    {
      id: "neak-ozd",
      badge: "NEAK · Ózd",
      title: "NEAK rendelő Ózd",
      note:
        "Külön városban működő NEAK irány. A közös előszobán belül tisztán el kell váljon a kazincbarcikai NEAK úttól.",
      tone: "Nyugodt, de helyszínhez kötött",
      focus: "Ózdi NEAK irány elkülönítése",
      nextStep: "A továbbhaladás előtt tisztázódik, hogy az ózdi NEAK út illeszkedik-e a látogató szándékához.",
      consult:
        "A konzultációs előszoba az ózdi NEAK irányhoz kötött első beszélgetési keretet mutathatja meg.",
      booking:
        "Az időpontkérés később az ózdi NEAK rendelő külön útvonalához és ritmusához igazodhat.",
      featured: false,
    },
  ] as const;
  const [activeRouteId, setActiveRouteId] = useState<(typeof clinicRoutes)[number]["id"]>(
    "private-ozd",
  );
  const activeRoute =
    clinicRoutes.find((route) => route.id === activeRouteId) ?? clinicRoutes[0];

  return (
    <div className="booking-gate-preview" aria-label="Időpontfoglalási kapu tervezett állapotban">
      <div className="booking-gate-preview__header">
      ...
      <div className="booking-gate-preview__steps" aria-hidden="true">
        <span />
        <span />
        <span />
      </div>
      <div className="booking-gate-preview__zones" aria-label="A következő rendelői világ zónái">
        <article className="booking-gate-preview__zone">
          ...
          <label className="booking-gate-preview__selector" htmlFor="clinic-route-select">
            <span>Rendelő kiválasztása</span>
            <select
              id="clinic-route-select"
              value={activeRoute.id}
              onChange={(event) =>
                setActiveRouteId(event.target.value as (typeof clinicRoutes)[number]["id"])
              }
            >
          ...
          <div
            className={`booking-gate-preview__selected-route${
              activeRoute.featured ? " booking-gate-preview__selected-route--featured" : ""
            }`}
            aria-live="polite"
          >
            <small>{activeRoute.badge}</small>
            <b>{activeRoute.title}</b>
            <p>{activeRoute.note}</p>
            <div className="booking-gate-preview__route-signals" aria-label="Kiválasztott rendelő hangulata">
              <article>
                <small>Hangulat</small>
                <b>{activeRoute.tone}</b>
              </article>
              <article>
                <small>Fókusz</small>
                <b>{activeRoute.focus}</b>
              </article>
              <article>
                <small>Következő lépés</small>
                <b>{activeRoute.nextStep}</b>
              </article>
            </div>
          </div>
          <div className="booking-gate-preview__routing-note">
        ...
        <article className="booking-gate-preview__zone">
          ...
          <p>{activeRoute.consult}</p>
        </article>
        <article className="booking-gate-preview__zone">
          <small>03 · Időablak előszoba</small>
          <b>Hogyan igazodjon a későbbi időpontkérés?</b>
          <p>{activeRoute.booking}</p>
        </article>
        <article className="booking-gate-preview__zone booking-gate-preview__zone--guard">
...
const technologyPathwaySteps = [
  {
    label: "01 · Bemeneti rend",
    title: "A labor nem géppel indul, hanem rendezett adattal",
    text:
      "A scan, fájl vagy egyéb digitális kiindulás itt még nem eredményt jelent, hanem olyan bemenetet, amelyet a labor előkészít, értelmez és munkaritmusba rendez.",
  },
  {
    label: "02 · Gyártási ritmus",
    title: "A CAD/CAM és a 3D út ugyanannak a folyamatnak állomása lehet",
    text:
      "A tervezés, marás, nyomtatás és köztes ellenőrzések nem külön reklámelemekként, hanem egymásra épülő munkaszakaszként jelennek meg a térben.",
  },
  {
    label: "03 · Minőségi perem",
    title: "Az anyag, az utómunka és az ellenőrzés külön kapuként válik láthatóvá",
    text:
      "A folyamat vége nem automatikus döntés. A whitebox itt annak a helyét mutatja meg, ahol később anyaghasználati, dokumentációs és szakmai kontrollpontok jelenhetnek meg.",
  },
] as const;

function TechnologyPathway() {
  return (
    <section aria-labelledby="technology-pathway-title" className="technology-pathway">
      <div className="technology-pathway__intro">
        <p className="scene-kicker">Szakmai laborút</p>
        <h2 id="technology-pathway-title">A beérkező adattól a következő szakmai kapuig</h2>
        <p>
          Ez a tér fogtechnikusoknak szóló, világosabb és követhetőbb
          laborvilágot mutat. A portálban látott fókusz itt tágul ki ugyanabba
          a munkatérbe: természetesebb fény, rendezett állomások és szakmailag
          olvasható munkaritmus mellett.
        </p>
      </div>
      <div aria-label="Fogtechnikai labor whitebox-útvonala" className="technology-pathway__steps">
        {technologyPathwaySteps.map((step) => (
          <article className="technology-pathway__step" key={step.title}>
      ...
      <aside className="technology-pathway__guard" aria-label="Fogtechnikai whitebox korlát">
        <strong>Whitebox-határ</strong>
        <p>
          Itt még nincs gépteljesítmény-ígéret, nincs automatikus
          anyagválasztás és nincs online szakmai döntés. A tér a laborfolyamat
          logikáját mutatja, nem végleges szakmai állítást.
        </p>
      </aside>
    </section>
  );
...
function HostConsole({
  ...
  return (
    <section aria-labelledby="host-console-title" className="host-console host-console--guidance">
      <div className="host-console__copy">
        <p className="scene-kicker">Kérdésalapú tudástér</p>
        <h2 id="host-console-title">Miben segíthetünk?</h2>
        <p>
          Ez a tér pácienseknek és érdeklődő látogatóknak készült. A látogató
          saját kérdéséből vagy helyzetéből indulhat el, majd átfogó
          tudástár-családok között tájékozódhat — adatbekérés, diagnózis és
          kezelési terv nélkül.
        </p>
      </div>
      <div aria-label="Látogatói eligazodási irányok" className="host-console__choices">
        <a className="host-console__choice" href="#knowledge-preview-title">
        ...
        <Link
          className="host-console__choice"
          href={scenes.clinics.path}
          onClick={(event) => onNavigate("clinics", event)}
        >
...
function KnowledgeArticle({
  ...
  return (
    <div
      className={`knowledge-capsule__article${
        variant === "secondary" ? " knowledge-capsule__article--secondary" : ""
      }`}
    >
      <div className="knowledge-capsule__article-header">
        <span>{article.status}</span>
        <h5>{article.title}</h5>
      </div>
      <p>{article.intro}</p>
      <div className="knowledge-capsule__article-points">
      ...
      <div className="knowledge-capsule__article-lists">
        <div>
      ...
      <div className="knowledge-capsule__article-sources">
...
function KnowledgePreview({
  ...
  const [activeTag, setActiveTag] =
    useState<(typeof guidanceTopicFamilies)[number]["tag"]>("01");
  const activeFamily =
    guidanceTopicFamilies.find((family) => family.tag === activeTag) ?? guidanceTopicFamilies[0];
  const activeDetail: GuidanceKnowledgeDraft = guidanceKnowledgeDrafts[activeFamily.tag];

  return (
    <section aria-labelledby="knowledge-preview-title" className="knowledge-preview">
      <div className="knowledge-preview__intro">
        <p className="scene-kicker">Tudástárfal vázlat</p>
        <h2 id="knowledge-preview-title">Kérdésből induló tudástér</h2>
        <p>
          A konkrét témák — például implantátum, fogsor, cirkónium vagy
          digitális tervezés — később ezek alatt jelennek meg kapszulaként. A
          fő rendszer most a látogató kérdéséből indul.
        </p>
      </div>
      <div className="knowledge-preview__cards" aria-label="Guidance tudástár-családok előnézete">
        {guidanceTopicFamilies.map((family) => (
          <article
            className={`knowledge-card${family.tag === activeFamily.tag ? " is-active" : ""}`}
            key={family.tag}
          >
            <span className="knowledge-card__tag">{family.tag}</span>
            <span className="knowledge-card__status">{family.status}</span>
            ...
            <button
              ...
              className="knowledge-card__select"
              onClick={() => setActiveTag(family.tag)}
              type="button"
            >
              {family.tag === activeFamily.tag ? "Megnyitva" : "Kapszula előnézet"}
            </button>
          </article>
        ))}
      </div>
      <aside
        ...
        aria-live="polite"
        className="knowledge-capsule"
        id="knowledge-capsule-panel"
      >
        <p className="scene-kicker">Kiválasztott tudáskapszula</p>
        <div className="knowledge-capsule__header">
          <span>{activeFamily.tag}</span>
          <h3 id="knowledge-capsule-title">{activeFamily.capsuleTitle}</h3>
        </div>
        <p>{activeFamily.capsuleLead}</p>
        <div className="knowledge-capsule__topics">
          <strong>Például ide kerülhet:</strong>
          <ul>
            {activeFamily.topics.map((topic) => (
              <li key={topic}>{topic}</li>
            ))}
          </ul>
        </div>
        <div className="knowledge-capsule__pathway">
          <strong>Első tudásútvonal</strong>
          <ol>
            {activeFamily.pathway.map((item, index) => (
              <li key={item.title}>
        ...
        <div className="knowledge-capsule__draft">
          <div className="knowledge-capsule__draft-header">
            <strong>Első részletes tudáskártya-minta</strong>
            <span>{activeDetail.status}</span>
          </div>
          <h4>{activeDetail.title}</h4>
          <p>{activeDetail.lead}</p>
          <div className="knowledge-capsule__draft-modules">
            {activeDetail.modules.map((module) => (
          ...
          <ul className="knowledge-capsule__draft-checks">
            {activeDetail.checks.map((check) => (
              <li key={check}>{check}</li>
            ))}
          </ul>
          {activeDetail.article ? <KnowledgeArticle article={activeDetail.article} /> : null}
          {activeDetail.relatedArticles?.map((article) => (
            <KnowledgeArticle article={article} key={article.title} variant="secondary" />
          ))}
        </div>
        <p className="knowledge-capsule__safety">{activeFamily.safety}</p>
        <Link
          className="knowledge-capsule__link"
          href={scenes.clinics.path}
          onClick={(event) => onNavigate("clinics", event)}
        >
          Tovább a Saját rendelők felé
        </Link>
      </aside>
    </section>
  );
...
function ClinicConsole() {
  return (
    <section aria-labelledby="clinic-console-title" className="host-console host-console--clinics">
      <div className="host-console__copy">
        <p className="scene-kicker">Rendelői előszoba</p>
        <h2 id="clinic-console-title">Innen indul a rendelői út</h2>
        <p>
          A guidance térben összerendezett kérdések itt rendelői előszobává
          alakulnak. A látogató szakmai beszélgetés felé léphet, de ez még nem
          online diagnózis, nem kezelési terv, nem panaszbeküldés és nem éles
          foglalás.
        </p>
      </div>
      <div aria-label="Saját rendelők előszobájának irányai" className="host-console__choices">
        <a className="host-console__choice" href="#clinics-stop-01">
          <span>Megérkezem a saját rendelőkbe</span>
          <small>Adatmentes rendelői fogadópont</small>
        </a>
        <a className="host-console__choice" href="#clinics-stop-02">
          <span>Szakmai beszélgetésre készülök</span>
          <small>A kérdéseim rendezése, online diagnózis nélkül</small>
        </a>
        <a className="host-console__choice" href="#clinics-stop-03">
...
function ClinicOrientation() {
  return (
    <section aria-labelledby="clinic-orientation-title" className="clinic-orientation">
      <div className="clinic-orientation__copy">
        <p className="scene-kicker">Rendelői orientáció</p>
        <h2 id="clinic-orientation-title">
          Nem űrlaphoz érkezik, hanem egy szakmai beszélgetés előterébe
        </h2>
        <p>
          A Saját rendelők tér feladata, hogy a tudástárból hozott bizonytalanságot
          nyugodt következő lépéssé alakítsa. A látogató itt még nem ad meg
          érzékeny adatot, és nem kap online döntést.
        </p>
      </div>
      <div className="clinic-orientation__map" aria-label="Saját rendelők orientációs út">
        {clinicOrientationPoints.map((point, index) => (
          <article className="clinic-orientation__point" key={point.label}>
...
function ClinicPathway() {
  return (
    <section aria-labelledby="clinic-pathway-title" className="clinic-pathway">
      <div className="clinic-pathway__intro">
        <p className="scene-kicker">Guidance után</p>
        <h2 id="clinic-pathway-title">A kérdésből rendelői út lesz</h2>
        <p>
          Ez a tér nem azonnali választ ad, hanem biztonságos következő lépést.
          A látogató a tudástárból hozott kérdéseit rendezettebben viheti tovább
          a saját rendelők felé.
        </p>
      </div>
      <div aria-label="Saját rendelők adatmentes útvonala" className="clinic-pathway__steps">
        {clinicPathwaySteps.map((step) => (
          <article className="clinic-pathway__step" key={step.title}>
            <small>{step.label}</small>
            <h3>{step.title}</h3>
            <p>{step.text}</p>
          </article>
      ...
      <aside className="clinic-pathway__guard" aria-label="Saját rendelők biztonsági kapu">
        <strong>Biztonsági kapu</strong>
        <p>
          Itt még nincs foglalási űrlap, nincs panaszbeküldés és nincs
          egészségügyi adatkezelés. A személyes szakmai döntés továbbra is
          rendelői konzultációhoz kötött.
        </p>
      </aside>
    </section>
  );
...
function TechnologyConsole() {
  return (
    <section aria-labelledby="technology-console-title" className="host-console host-console--technology">
      <div className="host-console__copy">
        <p className="scene-kicker">Digitális labor</p>
        <h2 id="technology-console-title">Fogtechnikai munkaritmus</h2>
        <p>
          Ez a szakmai tér ugyanazt a fogtechnikai világot nyitja ki, amely a
          HERO harmadik portáljában csak fókuszált részletként látszik. A
          whitebox most egy világosabb, természetesebb fényű laborritmust mutat:
          adat-előkészítés, CAD/CAM és 3D munkafázisok, artikulátoros
          ellenőrzési pontok és szakmai kontrollrétegek helyét.
        </p>
      </div>
      <div aria-label="Fogtechnikai labor irányai" className="host-console__choices">
        <a className="host-console__choice" href="#technology-stop-01">
          <span>Digitális adat előkészítése</span>
          <small>Rendezett bemeneti laborfolyamat</small>
        </a>
        <a className="host-console__choice" href="#technology-stop-02">
          <span>CAD/CAM gyártási ritmus</span>
          <small>Tervezés, marás, nyomtatás helye</small>
        </a>
        <a className="host-console__choice" href="#technology-stop-03">
...
function SceneVisual({ sceneId }: { sceneId: Exclude<SceneId, "hero"> }) {
  if (sceneId === "dentists") {
    return (
      <div aria-hidden="true" className="scene-visual scene-visual--dentists">
        <div className="dentist-workflow">
          <span className="dentist-workflow__arch" />
          <span className="dentist-workflow__glass dentist-workflow__glass--left" />
          <span className="dentist-workflow__glass dentist-workflow__glass--right" />
          <span className="dentist-workflow__chair" />
          <span className="dentist-workflow__scanner" />
          <span className="dentist-workflow__screen" />
          <span className="dentist-workflow__brand">DTA</span>
          <span className="dentist-workflow__tooth" />
          <span className="dentist-workflow__rail" />
          <span className="dentist-workflow__node dentist-workflow__node--one" />
          <span className="dentist-workflow__node dentist-workflow__node--two" />
          <span className="dentist-workflow__node dentist-workflow__node--three" />
          <span className="dentist-workflow__label dentist-workflow__label--one">Adat</span>
          <span className="dentist-workflow__label dentist-workflow__label--two">Tervezés</span>
          <span className="dentist-workflow__label dentist-workflow__label--three">Labor</span>
          <span className="dentist-workflow__lab" />
        </div>
      </div>
    );
  }
  ...
  if (sceneId === "guidance") {
    return (
      <div aria-hidden="true" className="scene-visual scene-visual--guidance">
        <div className="guidance-hub">
          <span className="guidance-hub__arch" />
          <span className="guidance-hub__welcome-ring" />
          <span className="guidance-hub__host-aura" />
          <span className="guidance-hub__knowledge-light guidance-hub__knowledge-light--left" />
          <span className="guidance-hub__knowledge-light guidance-hub__knowledge-light--right" />
          <span className="guidance-hub__clinic-portal" />
          <span className="guidance-hub__clinic-glow" />
          <span className="guidance-hub__desk" />
          <span className="guidance-hub__host" />
          <span className="guidance-hub__host-smile" />
          <span className="guidance-hub__console" />
          <span className="guidance-hub__prompt guidance-hub__prompt--one">Jó helyen jár</span>
          <span className="guidance-hub__prompt guidance-hub__prompt--two">Miben segíthetünk?</span>
          <span className="guidance-hub__search" />
          <span className="guidance-hub__library guidance-hub__library--left" />
          <span className="guidance-hub__library guidance-hub__library--right" />
          <span className="guidance-hub__route" />
          <span className="guidance-hub__node guidance-hub__node--one" />
          <span className="guidance-hub__node guidance-hub__node--two" />
          <span className="guidance-hub__node guidance-hub__node--three" />
          <span className="guidance-hub__label guidance-hub__label--one">Házigazda</span>
          <span className="guidance-hub__label guidance-hub__label--two">Tudástár</span>
          <span className="guidance-hub__label guidance-hub__label--three">Útvonal</span>
        </div>
      </div>
    );
  }
  ...
  if (sceneId === "clinics") {
    return (
      <div aria-hidden="true" className="scene-visual scene-visual--clinics">
        <div className="clinic-booking">
          <span className="clinic-booking__depth-wall clinic-booking__depth-wall--left" />
          <span className="clinic-booking__depth-wall clinic-booking__depth-wall--right" />
          <span className="clinic-booking__ceiling-light" />
          <span className="clinic-booking__floor-path" />
          <span className="clinic-booking__arch" />
          <span className="clinic-booking__desk" />
          <span className="clinic-booking__plant" />
          <span className="clinic-booking__brand">DTA</span>
          <span className="clinic-booking__waiting-seat" />
          <span className="clinic-booking__consult-room" />
          <span className="clinic-booking__calendar" />
          <span className="clinic-booking__service-panel" />
          <span className="clinic-booking__chair" />
          <span className="clinic-booking__clock" />
          <span className="clinic-booking__question-card" />
          <span className="clinic-booking__consultation" />
          <span className="clinic-booking__booking-gate" />
          <span className="clinic-booking__next-threshold" />
          <span className="clinic-booking__next-ring" />
          <span className="clinic-booking__next-room clinic-booking__next-room--left" />
          <span className="clinic-booking__next-room clinic-booking__next-room--right" />
          <span className="clinic-booking__next-seat" />
          <span className="clinic-booking__privacy-gate" />
          <span className="clinic-booking__route" />
          <span className="clinic-booking__node clinic-booking__node--one" />
          <span className="clinic-booking__node clinic-booking__node--two" />
          <span className="clinic-booking__node clinic-booking__node--three" />
          <span className="clinic-booking__label clinic-booking__label--one">Fogadópont</span>
          <span className="clinic-booking__label clinic-booking__label--two">Beszélgetés</span>
          <span className="clinic-booking__label clinic-booking__label--three">Zárt kapu</span>
        </div>
      </div>
    );
  }
  ...
  if (sceneId === "technology") {
    return (
      <div aria-hidden="true" className="scene-visual scene-visual--technology">
        <div className="tech-lab">
          <span className="tech-lab__skylight" />
          <span className="tech-lab__daylight" />
          <span className="tech-lab__window-band" />
          <span className="tech-lab__window-panel tech-lab__window-panel--one" />
          <span className="tech-lab__window-panel tech-lab__window-panel--two" />
          <span className="tech-lab__shelf tech-lab__shelf--left" />
          <span className="tech-lab__shelf tech-lab__shelf--center" />
          <span className="tech-lab__lounge" />
          <span className="tech-lab__island" />
          <span className="tech-lab__monitor" />
          <span className="tech-lab__printer tech-lab__printer--one" />
          <span className="tech-lab__printer tech-lab__printer--two" />
          <span className="tech-lab__scanner" />
          <span className="tech-lab__technician tech-lab__technician--one" />
          <span className="tech-lab__technician tech-lab__technician--two" />
          <span className="tech-lab__bench" />
          <span className="tech-lab__articulator tech-lab__articulator--one" />
          <span className="tech-lab__articulator tech-lab__articulator--two" />
          <span className="tech-lab__articulator tech-lab__articulator--three" />
          <span className="tech-lab__models" />
          <span className="tech-lab__route" />
          <span className="tech-lab__node tech-lab__node--one" />
          <span className="tech-lab__node tech-lab__node--two" />
          <span className="tech-lab__node tech-lab__node--three" />
          <span className="tech-lab__label tech-lab__label--one">Bemenet</span>
          <span className="tech-lab__label tech-lab__label--two">Gyártás</span>
          <span className="tech-lab__label tech-lab__label--three">Kontroll</span>
        </div>
      </div>
    );
  }
  ...
  return (
    <div aria-hidden="true" className="scene-visual scene-visual--generic">
      <div className="whitebox-object whitebox-object--primary" />
      <div className="whitebox-object whitebox-object--secondary" />
    </div>
  );
}
